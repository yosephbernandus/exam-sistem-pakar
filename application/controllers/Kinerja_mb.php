...
class Kinerja_mb extends CI_Controller {
	...
	public function index(){
		$data['view_pertanyaan'] = $this->model_mb_k->view_pertanyaan()->result();
		$data['view_solusi'] = $this->model_mb_k->view_solusi()->result();
		$data['view_all'] = $this->model_mb_k->view_all()->result();
		...
		$this->template->display('backend/kinerja_mb/index',$data);
	}
	...
	public function tambah_pertanyaan(){
		...
		if ($this->form_validation->run()==true) {
			...
			$cek=$this->model_mb_k->cek($pertanyaan);
			if ($cek->num_rows()>0) {
				$data['message']="<div class='alert alert-warning'>ID Already Use</div>";
				$this->template->display('backend/kinerja_mb/tambah_pertanyaan',$data);
			} else {
				$info=array(
					'pertanyaan'=>$this->input->post('pertanyaan'),
				...
				$this->model_mb_k->simpan($info);
				redirect('kinerja_mb/index/add_success_pertanyaan');
			}
		} else {
			$data['message']="";
			$data['view_all_result'] = $this->model_mb_k->view_all()->result();
			$this->template->display('backend/kinerja_mb/tambah_pertanyaan',$data);
		}
	}
	...
	public function tambah_solusi(){
		...
		if ($this->form_validation->run()==true) {
			...
			if ($cek->num_rows()>0) {
				...
				$this->model_mb_k->simpan($info);
				redirect('kinerja_mb/index/add_success_solusi');
			}
		} else {
			$data['message']="";
			$this->template->display('backend/kinerja_mb/tambah_solusi',$data);
		}
	}
	...
	public function edit($id_pertanyaan){
		...
		if ($this->form_validation->run()==true) {
			...
			$this->model_mb_k->update($id_pertanyaan,$simpan);
			$data['message']="<div class='alert alert-success'>Data Updated</div>";
			$data['view_all']=$this->model_mb_k->cek($id_pertanyaan)->row_array();
			$this->template->display('backend/kinerja_mb/edit',$data);
		} else {
			$data['view_all']=$this->model_mb_k->cek($id_pertanyaan)->row_array();
			$data['view_all_result'] = $this->model_mb_k->view_all()->result();
			$data['message']="";
	...
	public function edit_solusi($id_pertanyaan){
		...
		if ($this->form_validation->run()==true) {
			...
			$data['message']="<div class='alert alert-success'>Data Updated</div>";
			$data['view_all']=$this->model_mb_k->cek($id_pertanyaan)->row_array();
			$this->template->display('backend/kinerja_mb/edit_solusi',$data);
		} else {
			$data['view_all']=$this->model_mb_k->cek($id_pertanyaan)->row_array();
			$data['view_all_result'] = $this->model_mb_k->view_all()->result();
			$data['message']="";
