...
class Sata extends CI_Controller {
	...
	public function index(){
		$data['view_pertanyaan'] = $this->model_sata->view_pertanyaan()->result();
		$data['view_solusi'] = $this->model_sata->view_solusi()->result();
		$data['view_all'] = $this->model_sata->view_all()->result();
		...
		$this->template->display('backend/sata/index',$data);
	}
	...
	public function tambah_pertanyaan(){
		...
		if ($this->form_validation->run()==true) {
			...
			$cek=$this->model_sata->cek($pertanyaan);
			if ($cek->num_rows()>0) {
				$data['message']="<div class='alert alert-warning'>ID Already Use</div>";
				$this->template->display('backend/sata/tambah_pertanyaan',$data);
			} else {
				$info=array(
					'pertanyaan'=>$this->input->post('pertanyaan'),
				...
				$this->model_sata->simpan($info);
				redirect('sata/index/add_success_pertanyaan');
			}
		} else {
			$data['message']="";
			$data['view_all_result'] = $this->model_sata->view_all()->result();
			$this->template->display('backend/sata/tambah_pertanyaan',$data);
		}
	}
	...
	public function tambah_solusi(){
		...
		if ($this->form_validation->run()==true) {
			...
			if ($cek->num_rows()>0) {
				...
				$this->model_sata->simpan($info);
				redirect('sata/index/add_success_solusi');
			}
		} else {
			$data['message']="";
			$this->template->display('backend/sata/tambah_solusi',$data);
		}
	}
	...
	public function edit($id_pertanyaan){
		...
		if ($this->form_validation->run()==true) {
			...
			$this->model_sata->update($id_pertanyaan,$simpan);
			$data['message']="<div class='alert alert-success'>Data Updated</div>";
			$data['view_all']=$this->model_sata->cek($id_pertanyaan)->row_array();
			$this->template->display('backend/sata/edit',$data);
		} else {
			$data['view_all']=$this->model_sata->cek($id_pertanyaan)->row_array();
			$data['view_all_result'] = $this->model_sata->view_all()->result();
			$data['message']="";
	...
	public function edit_solusi($id_pertanyaan){
		...
		if ($this->form_validation->run()==true) {
			...
			$data['message']="<div class='alert alert-success'>Data Updated</div>";
			$data['view_all']=$this->model_sata->cek($id_pertanyaan)->row_array();
			$this->template->display('backend/sata/edit_solusi',$data);
		} else {
			$data['view_all']=$this->model_sata->cek($id_pertanyaan)->row_array();
			$data['view_all_result'] = $this->model_sata->view_all()->result();
			$data['message']="";
